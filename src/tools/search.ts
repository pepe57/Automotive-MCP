...
export function searchRequirements(db: InstanceType<typeof Database>, input: SearchRequirementsInput): SearchResult[] {
  ...
  try {
    ...
    // Only search supplementary content types when no source filter is active.
    // The sources filter targets regulation/standard IDs (e.g., "r155", "iso_21434"),
    // which don't apply to architecture patterns, attack patterns, or CSMS obligations.
    const searchAllContentTypes = !sources || sources.length === 0;

    if (searchAllContentTypes) {
